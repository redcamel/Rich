...
let defineCustomType = (target, keyName, type, option) => {
    let tempDefineInfo;
    // 옵션 상태 체크
        ...
        set: function (v) {
            if (!(v instanceof type)) {
                if (!(NULLISH_ABLE && (v === null || v === undefined))) {
                    ...
                    else throwError(`${type.name} - v ${type.name}만 허용함. / 입력값 : ${v}`);
                }
            }
            this['_' + keyName] = v
            // 콜백 옵션실행
    ...
    // 기본값 생성
    if (NULLISH_ABLE && !option.hasOwnProperty('value')) option['value'] = null
    // 타입형 체크
    if (!(option['value'] instanceof type)) {
        if (NULLISH_ABLE && (option['value'] == null || option['value'] === undefined)) {
        } else {
            if (option['value'] == null || option['value'] === undefined) {
                throwError(`${target.name} - option['value'] : nullish를 허용하지 않는 세팅상태. / 입력값 : ${option['value']}`);
            } else {
                throwError(`${type.name} - option['value'] : ${type.name}만 허용함. / 입력값 : ${option['value']}`);
            }
        }
    }
    // 초기값 지정
